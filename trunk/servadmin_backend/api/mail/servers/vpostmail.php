<?
	/*
		VPostMail Mail-Handling class
		(a postfix handler I wrote)
		by Jon Coulter
	*/
...
class ServerAdminMail extends ServerAdminBase
{
	...
	private $vpostmail = '/usr/sbin/vpostmail';
...
//	}
	...
	function domainExists( $domain ) {
		...
		return (in_array($domain, $allDomains) ? true : false);
	}
...
//		);
		...
		$allUsers = $this->getUsers($domain);

		return in_array($u, $allUsers) ? true : false;
	}

	function getUsers( $domain ) {
		$ret = $this->execute('vpostmail',
			array('listusers', $domain),
			0,
			true
		);
...
//		}
		...
		$ret = $this->execute('vpostmail', array('adduser', $u, $passwd), 0, true);
	...
	{
		$this->raiseError("not yet implemented for vpostmail");
