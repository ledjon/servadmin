<?
	...
	$base_domain = 'foobar%d.com';

	/* current domains */
	print_r($mail->getDomains( ));

for($i = 0; $i < 50; $i++) {
	$domain = sprintf($base_domain, $i);
	...
	if(! $mail->domainAliasExists( $domain, 'myfakedomain.com' ) ) {
		$mail->addDomainAlias( $domain, 'myfakedomain.com' )
			or die("Unable to create domain alais");

		// remove it now
		$mail->delDomainAlias( $domain, 'myfakedomain.com' )
